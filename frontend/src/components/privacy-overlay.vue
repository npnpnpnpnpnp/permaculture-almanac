<template>
  <transition name="t-view">
    <section v-if="isVisible" :class="$style.component" :style="spacingTop">
      <base-bodytext :text="privacy" :class="$style.privacy" />
      <div>
        <button type="button" :class="$style.button" @click="closePrivacy" />
      </div>
    </section>
  </transition>
</template>
...
export default {
  ...
  methods: {
    closePrivacy() {
      this.$emit('privacy-visible', false)
    }
  }
}
</script>
